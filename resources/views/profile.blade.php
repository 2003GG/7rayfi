...
<body>
      ...
      <div>
          ...
          <div class="hero-content">
            <div class="hero-top-row">
              <div>
                <div class="hero-avatar">
                  @if(auth()->user()->profile_photo)
                    <img src="{{ asset('image/' . auth()->user()->profile_photo) }}" alt="Profile Photo">
                  @else
              ...
              <div style="display:flex; flex-direction:column; align-items:flex-end; gap:8px;">
                <button class="connect-btn" onclick="document.getElementById('edit-modal').classList.remove('hidden')">
        ...
        <div class="section-card">
          ...
          <div class="section-header">
            <h3 style="font-family:'Cinzel',serif; font-size:13px;">À Propos</h3>
            <button class="add-btn">✎ Modifier</button>
          </div>
          <div class="section-body">
            <p style="font-size:14px; color:var(--ink-dim); line-height:1.8;">
              {{ auth()->user()->biographie ?? 'Artisan passionné depuis plus de 15 ans, spécialisé dans les arts traditionnels marocains. Mon atelier à Marrakech perpétue des techniques ancestrales transmises de génération en génération.' }}
            </p>
          </div>
        </div>
        ...
        <div class="section-card" style="margin-bottom:0;">
          ...
          <div class="section-body">
            ...
            <div style="display:flex; align-items:center; gap:10px; padding:6px 0;">
              ...
              <span style="font-size:12px; color:var(--ink-dim); overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">{{ auth()->user()->email }}</span>
            </div>
            <div style="display:flex; align-items:center; gap:10px; padding:6px 0;">
              <div style="width:28px; height:28px; border-radius:8px; background:var(--surface3); display:flex; align-items:center; justify-content:center; flex-shrink:0;">
              ...
              <span style="font-size:12px; color:var(--ink-dim);">{{ auth()->user()->phone_number ?? 'Non renseigné' }}</span>
            </div>
          </div>
        </div>
  ...
  <div id="edit-modal" class="edit-modal-ov hidden" onclick="if(event.target===this) document.getElementById('edit-modal').classList.add('hidden')">
    <div class="edit-modal-box" style="overflow-y:auto; max-height:90vh;">
      ...
      <!-- Form -->
      <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data" style="padding:20px 22px;">
        @csrf @method('PUT')
        ...
        <div style="display:flex; flex-direction:column; align-items:center; margin-bottom:20px; padding-bottom:20px; border-bottom:1px solid var(--border);">
          <div style="position:relative; margin-bottom:8px;">
            <div style="width:80px; height:80px; border-radius:50%; overflow:hidden; border:2px solid var(--saffron);">
              @if(auth()->user()->profile_photo)
                <img id="modal-avatar-img" src="{{ asset('image/' . auth()->user()->profile_photo) }}" style="width:100%; height:100%; object-fit:cover;">
              @else
                <img id="modal-avatar-img" src="{{ asset('image/profilDefault.jpg') }}" style="width:100%; height:100%; object-fit:cover;">
              @endif
            </div>
            <label for="modal-photo-input" style="position:absolute; bottom:0; right:0; width:26px; height:26px; border-radius:50%; background:linear-gradient(135deg,var(--clay),var(--saffron)); display:flex; align-items:center; justify-content:center; cursor:pointer; border:2px solid var(--surface2);">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#0e0b08" stroke-width="2.5"><path d="M23 19a2 2 0 01-2 2H3a2 2 0 01-2-2V8a2 2 0 012-2h4l2-3h6l2 3h4a2 2 0 012 2z"/><circle cx="12" cy="13" r="4"/></svg>
            </label>
          </div>
          <input type="file" name="profile_photo" id="modal-photo-input" accept="image/*" style="display:none;" onchange="previewModalPhoto(event)" />
          <span style="font-size:11px; color:var(--ink-muted); font-family:'Cinzel',serif; letter-spacing:0.5px;">Changer la photo</span>
        </div>

        <!-- ── Fields ── -->
        <div style="margin-bottom:14px;">
          <label class="fl">Nom complet</label>
          <input type="text" name="name" value="{{ auth()->user()->name }}" class="fi" />
        </div>
        <div style="margin-bottom:14px;">
          <label class="fl">Titre professionnel</label>
          <input type="text" name="headline" value="{{ auth()->user()->headline ?? '' }}" placeholder="ex: Maître Artisan Zellige · Marrakech" class="fi" />
        </div>
        <div style="margin-bottom:14px;">
          <label class="fl">Localisation</label>
          <input type="text" name="localisation" value="{{ auth()->user()->localisation ?? '' }}" placeholder="ex: Marrakech, Maroc" class="fi" />
        </div>
        <div style="margin-bottom:14px;">
          <label class="fl">Biographie</label>
          <textarea name="biographie" rows="3" placeholder="Décrivez votre parcours artisanal…" class="fi fta">{{ auth()->user()->biographie ?? '' }}</textarea>
        </div>
        <div style="margin-bottom:20px;">
          <label class="fl">Téléphone</label>
          <input type="text" name="phone_number" value="{{ auth()->user()->phone_number ?? '' }}" placeholder="+212 6XX XXX XXX" class="fi" />
        </div>

        <!-- ── Buttons ── -->
        <div style="display:flex; gap:10px; justify-content:flex-end;">
          <button type="button" onclick="document.getElementById('edit-modal').classList.add('hidden')" class="btn-cancel">Annuler</button>

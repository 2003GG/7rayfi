...
<body>
  ...
  <div class="page-wrap">
    ...
    {{-- ✅ Flash messages --}}
    @if(session('error'))
      <div id="flash-error" style="display:flex; align-items:center; justify-content:space-between; gap:10px; background:rgba(220,50,50,0.1); border:1px solid rgba(220,50,50,0.3); border-radius:12px; padding:12px 18px; margin-bottom:20px; font-family:'Tajawal',sans-serif; font-size:13px; color:#e55;">
        <div style="display:flex; align-items:center; gap:10px;">
          <svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
          {{ session('error') }}
        </div>
        <button onclick="document.getElementById('flash-error').remove()" style="background:none; border:none; cursor:pointer; color:#e55; font-size:16px; line-height:1;">×</button>
      </div>
    @endif

    @if(session('success'))
      <div id="flash-success" style="display:flex; align-items:center; justify-content:space-between; gap:10px; background:rgba(80,200,120,0.1); border:1px solid rgba(80,200,120,0.3); border-radius:12px; padding:12px 18px; margin-bottom:20px; font-family:'Tajawal',sans-serif; font-size:13px; color:#50c878;">
        <div style="display:flex; align-items:center; gap:10px;">
        ...
        <button onclick="document.getElementById('flash-success').remove()" style="background:none; border:none; cursor:pointer; color:#50c878; font-size:16px; line-height:1;">×</button>
      </div>
    @endif
        ...
        <div class="products-grid" id="products-grid">
          ...
          @empty
            <div style="grid-column:1/-1; text-align:center; padding:60px 20px; color:var(--ink-muted);">
              ...
              <p style="font-family:'Cinzel',serif; font-size:16px; color:var(--ink); margin-bottom:8px;">Aucun produit disponible</p>
              <p style="font-size:13px;">Soyez le premier à publier un produit !</p>
            </div>
          @endforelse
    ...
    <div style="background:var(--surface2); border:1px solid var(--border); border-radius:22px; width:100%; max-width:560px; overflow-y:auto; max-height:90vh; position:relative;">
      ...
      <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data" style="padding:24px 28px; display:flex; flex-direction:column; gap:18px;">
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px;">
          <div>
            <label class="filter-section-title" style="display:block; margin-bottom:6px;">Prix (points) *</label>
  ...
  <script>
    ...
    setTimeout(function() {
      ...
      var suc = document.getElementById('flash-success');
      if (err) err.remove();
      if (suc) suc.remove();
    }, 4000);
    ...
    function toggleWishlist(btn) {
      ...
      svg.setAttribute('fill', btn.classList.contains('liked') ? '#e05555' : 'none');
    }
    ...
    document.querySelectorAll('.view-btn').forEach(function(btn) {
      btn.addEventListener('click', function() {
        ...
        grid.style.gridTemplateColumns = btn.title === 'Liste' ? '1fr' : 'repeat(3, 1fr)';
      });
    });

    /* Quick view */
    var qvQty = 1;
    function openQuickView(name, price, desc) {
    ...
    function changeQty(d) { qvQty = Math.max(1, qvQty + d); document.getElementById('qv-qty').textContent = qvQty; }
